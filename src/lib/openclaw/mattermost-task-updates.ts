...
// Only the three explicitly approved exception/completion families may speak in
...
const semanticMessages:Record<string,(title:string,detail:string)=>string>={
  ...
  task_exception:(title,detail)=>`⚠️ **CEO exception — ${title}**\n${detail}`,
  completion_rework:(title,detail)=>`↩️ **Completion evidence needs rework — ${title}**\n${detail}`,
  executive_synthesis:(title,detail)=>`✅ **Executive completion synthesis — ${title}**\n${detail}`,
};
...
export async function drainMattermostOutbox(now=new Date(),sender=defaultMattermostSender):Promise<{delivered:number;failed:number}>{
  ...
  const ids=queryAll<{id:string}>("SELECT id FROM mattermost_task_update_outbox WHERE state='pending' AND milestone IN ('task_exception','completion_rework','executive_synthesis') AND attempts<? AND (not_before IS NULL OR not_before<=?) ORDER BY created_at",[MAX_ATTEMPTS,iso]);
