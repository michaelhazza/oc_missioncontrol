...
- Delivery binds the originating Mattermost account, channel, and root as one atomic destination. Workers and outbox senders must never default to their current account or current conversation; a cross-account fallback can pair a valid root with the wrong DM channel and is rejected by Mattermost.
...
Mission Control migrations are additive and preserve existing non-Mattermost and legacy tasks. Migration 039 backfills the outbox account from each task's canonical origin (legacy rows default to `switch`). Intake delivery is replay-safe across restarts; the unique provider event and canonical-thread indexes prevent duplicate tasks. Thread outbox retries are bounded and retain diagnostics on terminal failure.
