...
test('fenced outbox delivers only allowlisted semantic families',async()=>{
  ...
  const result=await updates.drainMattermostOutbox(new Date(t0.getTime()+5_000),async item=>{sends++;observed=item;return{messageId:'post-1'}});
  assert.deepEqual(result,{delivered:3,failed:0});assert.equal(sends,3);assert.equal(observed.milestone,'executive_synthesis');
  assert.deepEqual(await updates.drainMattermostOutbox(new Date(t0.getTime()+1_000),async()=>{sends++;return{}}),{delivered:0,failed:0});assert.equal(sends,3);
  ...
  assert.equal(db.queryOne<{n:number}>("SELECT COUNT(*) n FROM task_activities WHERE task_id='threaded' AND activity_type='mattermost_update_delivered'")!.n,3);
});
